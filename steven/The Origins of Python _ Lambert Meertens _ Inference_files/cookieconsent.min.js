    ...
    if (!t.isInit) {
        ...
        t.fadeEnd = function () {
            ...
            var n = {
                ...
                container: '<div class="wpcc-container {{classes}} uk-box-shadow-medium">{{children}}</div>',
                template: '<span class="wpcc-message">{{message}} <a class="wpcc-privacy ct-button ct-button-underlined uk-display-inline-block" href="{{href}}" rel="noopener" target="{{target}}">{{link}}</a></span><div class="wpcc-compliance"><a class="wpcc-btn ct-button ct-button-outlined ct-fill-infered-light">{{button}}</a></div>',
                pushdown: !1,
